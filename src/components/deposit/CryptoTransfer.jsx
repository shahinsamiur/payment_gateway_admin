...
const CryptoTransfer = ({ status, paymentMethod }) => {
  const [page, setpage] = useState(1);
  const { data, isLoading } = useGetCryptoTransferDepositQuery({
    status,
    page,
  ...
  async function hadleStatusUpdate(id, status) {
    try {
      ...
      dispatch(apiSlice.util.invalidateTags(["crypto-transfer-deposit"]));
