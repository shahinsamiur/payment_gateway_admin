...
const MobileTransfer = ({ page, status, paymentMethod }) => {
  const { data, isLoading } = useGetMobileBankingWithdrawQuery({
    status,
    page,
  ...
  async function hadleStatusUpdate(id, status) {
    try {
      ...
      dispatch(apiSlice.util.invalidateTags(["mobile-banking-withdraw"]));
            ...
            {isLoading ? (
              ...
              data?.data?.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>{item.payment_method_gateway}</TableCell>
