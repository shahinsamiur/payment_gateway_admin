...
const WithdrawTransactions = ({ status, paymentMethod, tnxId }) => {
  const [showDetails, setShowDetails] = useState(0);
  const [page, setpage] = useState(1);
  const [checkingId, setCheckingId] = useState(0);
  const { data, isLoading } = useGetWithdrawHitoriesQuery({
    status,
    page,
    payment_system: paymentMethod,
    transaction_id: tnxId,
  });
  const [recheckStatus] = useRecheckApayWithdrawTransactionStatusMutation();
    ...
    <>
      ...
      {!!showDetails && (
        <WithdrawDetails
          ...
          title={`Apay transaction details - Withdraw`}
        />
      )}
    </>
  );
